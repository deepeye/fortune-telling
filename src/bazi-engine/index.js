...
function birthHourToClockHour(birthHour) {
  // Use midpoint of the 时辰 range as representative clock hour
  const [start, end] = HOUR_RANGES[birthHour];
  // 子时 spans 23:00-01:00, use 0 (midnight) as midpoint
  if (birthHour === 0) return 0;
  return (start + end) / 2;
}

function resolveToSolar(birthDate, calendarType) {
...
function computeBaZiProfile({ birthDate, birthHour, calendarType }) {
  const solar = resolveToSolar(birthDate, calendarType);
  const isHourEstimated = birthHour === -1;
  const resolvedHour = isHourEstimated ? 0 : birthHour;
  const clockHour = birthHourToClockHour(resolvedHour);
  const solarWithTime = Solar.fromYmdHms(
    solar.getYear(), solar.getMonth(), solar.getDay(), clockHour, 0, 0
  );
  const lunar = solarWithTime.getLunar();
  const baZi = lunar.getBaZi();

  return {
    yearPillar: { stem: baZi[0][0], branch: baZi[0][1] },
    monthPillar: { stem: baZi[1][0], branch: baZi[1][1] },
    dayPillar: { stem: baZi[2][0], branch: baZi[2][1] },
    hourPillar: { stem: baZi[3][0], branch: baZi[3][1] },
    dayMaster: STEM_TO_ELEMENT[baZi[2][0]],
    fiveElementsDistribution: computeDistribution(baZi),
    isHourEstimated,
  };
}

function computeDistribution(baZi) {
  const dist = { 木: 0, 火: 0, 土: 0, 金: 0, 水: 0 };
  for (const pillar of baZi) {
    dist[STEM_TO_ELEMENT[pillar[0]]]++;
    dist[BRANCH_TO_ELEMENT[pillar[1]]]++;
  }
  return dist;
}

// Five Elements generating/controlling relationships
// 生(generate): 木→火→土→金→水→木
// 克(control): 木→土→水→火→金→木
const GENERATES = { 木: "火", 火: "土", 土: "金", 金: "水", 水: "木" };
const CONTROLS = { 木: "土", 土: "水", 水: "火", 火: "金", 金: "木" };

// Score contribution of an element relative to Day Master
// 生DM (element generates Day Master): +3
// 同DM (same as Day Master): +2
// DM克此 (Day Master controls this): +1
// DM生此 (Day Master generates this): 0
// 此克DM (this controls Day Master): -2
...
function computeYearlyTrend(profile, startYear) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // Use Jan 15 of each year as representative date for yearly scoring
...
module.exports = { computeBaZiProfile, computeDailyFortune, generateFortuneTip, generateLuckyRecommendations, computeYearlyTrend };